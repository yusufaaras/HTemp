...
export default function EmailSignIn({ allowPassword, redirectMethod }: EmailSignInProps) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        await handleRequest(e, signInWithEmail, redirectMethod === 'client' ? router : null); // Koşulsuz kullanım
        ...
        <div className="mb-8">
            <form noValidate={true} className="mb-4" onSubmit={(e) => handleSubmit(e)}>
                <div className="grid gap-2">
                    ...
                    <Button type="submit" className="mt-2 flex h-[unset] w-full items-center justify-center px-4 py-4 text-sm font-medium">
                        {isSubmitting ? (
                            <svg
                                aria-hidden="true"
                                role="status"
            ...
            {allowPassword && (
                <>
                    <p>
                        <Link href="/dashboard/signin/password_signin" className="font-medium text-sm dark:text-white">
