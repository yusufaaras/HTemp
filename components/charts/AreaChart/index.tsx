...
export default function AreaChartComponent() {
  ...
  function handleTimeRange(value:string){
    setTimeRange(value)
  }
    ...
    <Card className="w-full items-center justify-between rounded-md border-zinc-200 bg-clip-border dark:border-zinc-800">
      <CardHeader className="flex items-start md:items-center justify-between gap-4 space-y-0 py-5 sm:flex-row">
        <div className="flex h-14 w-14 items-center justify-center rounded-full border border-zinc-200 text-4xl dark:border-zinc-800 text-foreground dark:text-white">
          <HiChartBar className="h-5 w-5 text-foreground dark:text-white" />
        </div>
        ...
        <Select value={timeRange} onValueChange={(value) => setTimeRange(value)}>
          <SelectTrigger
            className="w-[160px] rounded-md sm:ml-auto text-foreground dark:text-white"
            aria-label="Select a value"
          >
            <SelectValue placeholder="Last 3 months" />
          </SelectTrigger>
          <SelectContent className="rounded-lg z-10 bg-white dark:bg-zinc-950 dark:border-zinc-800">
            <SelectItem value="90d" className="rounded-md text-foreground dark:text-white hover:dark:text-white">
              Last 3 months
            </SelectItem>
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        <ChartContainer
          config={chartConfig}
          className="aspect-auto h-[300px] xl:h-[200px] 2xl:h-[300px] w-full"
        >
          <AreaChart data={filteredData}>
            ...
            />
            <ChartLegend
              className="text-xs font-medium leading-5 text-zinc-500 dark:text-white"
